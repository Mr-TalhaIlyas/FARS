...
from core.bricks import DownSample, LayerScale, StochasticDepth, DWConv3x3, NormLayer
...
class BlockFFN(nn.Module):
    def __init__(self, in_channels, out_channels, hid_channels, ls_init_val=1e-2,
                 drop_path=0., drop_path_mode='row'):
        super().__init__()
        ...
        self.drop_path = StochasticDepth(p=drop_path, mode=drop_path_mode)
...
class BlockMSCA(nn.Module):
    def __init__(self, dim, ls_init_val=1e-2, drop_path=0.0, drop_path_mode='row'):
        super().__init__()
...
class StageMSCA(nn.Module):
    def __init__(self, dim, ffn_ratio=4., ls_init_val=1e-2, drop_path=0.0, drop_path_mode='row'):
        super().__init__()
        # print(f'StageMSCA {drop_path}')
        self.msca_block = BlockMSCA(dim, ls_init_val, drop_path, drop_path_mode)
        ...
        self.ffn_block = BlockFFN(in_channels=dim, out_channels=dim,
                                  ...
                                  drop_path=drop_path, drop_path_mode=drop_path_mode)
...
class MSCANet(nn.Module):
    def __init__(self, in_channnels=3, embed_dims=[32, 64, 460,256],
                 ...
                 ls_init_val=1e-2, drop_path=0.0, drop_path_mode='row'):
