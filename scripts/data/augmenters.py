...
E = config['step_epoch']
E.append(np.inf)
p1 = config['geometric_aug']
p2 = config['noise_aug']
# probab for IDENTITY funtion
p3 = np.where(np.asarray(p1)==0, 1, 0)
p3 = list(p3)
...
def IDENTITY(img, lbl):
    pass
    return img, lbl

def get_prob(current_epoch, E, p1, p2, p3):

    for j in range(len(E)):
        if current_epoch >= E[j] and current_epoch < E[j+1]:
            a, b, c = p1[j], p2[j], p3[j]
        # print(a, b, c, d)
    return a, b, c

def data_augmenter(img_patch, lbl_patch, epoch):
    
    func_args = [
                (geomatric_augs, (img_patch, lbl_patch)),
                (noise_augs,     (img_patch, lbl_patch)),
                (IDENTITY,       (img_patch, lbl_patch))
                ]
    #print('one iter done')
    a, b, c = get_prob(epoch, E, p1, p2, p3)
    (func, args), = random.choices(func_args, weights=[a,b,c])
    img_patch, lbl_patch = func(*args)                

    return img_patch, lbl_patch, a, b
