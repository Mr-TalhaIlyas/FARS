...
from utils import Trainer_PSP, Evaluator_PSP, ModelUtils, eval_wrapper
...
loss = FocalLoss()
aux_loss = nn.CrossEntropyLoss(ignore_index=0)
criterion = lambda x,y: loss(x, y)
aux_criterion = lambda x,y: aux_loss(x, y)
...
trainer = Trainer_PSP(model, config['batch_size'], optimizer, criterion, aux_loss, metric)
# trainer = Trainer(model, config['batch_size'], optimizer, metric)
evaluator = Evaluator_PSP(model, metric)
trg_evaluator = Evaluator_PSP(model, metric)
# Initializing plots
if config['LOG_WANDB']:
